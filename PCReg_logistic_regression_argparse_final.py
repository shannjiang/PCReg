...
parser.add_argument('--covars', nargs='+',
                    help='covariates for logistic regression')
parser.add_argument('--dep_var', type=str,
                    help='dependent variable for logistic regression')
...
def PC_LogisticRegression(retain_PCs_df, vars_df, covar, dep_var, fit_method='bfgs'):
    ...
    if covar == ['None']:
        ...
        X = reg_df[X_col]
        y = reg_df[y_col]
        full_log_res = sm.Logit(y, X).fit(method=fit_method)
